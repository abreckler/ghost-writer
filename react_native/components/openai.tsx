...
class GhostWriterConfig {
  ...
  public static generateCompleteParams(seedText: string, writingMode?: string): CompletionParams {
    ...
    {
      const STOP_SEQ = '"';
      params.prompt = 'Someone wrote as: ' + STOP_SEQ + seedText.trim() + STOP_SEQ +
                      '\nAnd another person wrote on the same subject matter as: ' + STOP_SEQ;
      params.stop = [ STOP_SEQ ];
    ...
    {
      const STOP_SEQ = '"""';
      params.prompt = 'Question: ' + STOP_SEQ + seedText.trim() + STOP_SEQ +
                      '\nAnswer: ' + STOP_SEQ;
      params.stop = [
        STOP_SEQ, 'Question:', 'Q:', // Possible separation of blocks generated by the engine.
